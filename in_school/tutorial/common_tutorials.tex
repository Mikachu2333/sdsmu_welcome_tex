...
\subsection[新生信息查询、线上报到]{\uul{新生信息查询、线上报到}}
...
\begin{enumerate}
    ...
    \item →查看宿舍号、学号、班级、院系等相关信息，并根据个人需求选择是否订购军训套装、被褥套装\footnotemark 等
          \footnotetext{如自带则必须按学校有关规定购买尺寸、颜色一致的样式；详请参见\uref{bedding_set}与录取通知相关材料等说明。}
    \item 智慧学工系统的操作办法同上
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{警告：}一个账号最多允许\textbf{4}台设备同时在线
    \item \textbf{严禁向他人共享校园网账号及一切校内资源}
\end{enumerate}

\subsubsection[有线连接]{有线连接}
\begin{enumerate}
    \item 按照\uref{wifi_register}的教程激活校园网
...
\begin{enumerate}
    \item 确认现在已连接校园网
    \item 在浏览器中打开\uhref{https://slzfw.sdsmu.edu.cn:8800}{https://slzfw.sdsmu.edu.cn:8800$^㊕$}，并登录
    \item 点击左侧菜单栏“用户”→“无感知认证”
    \item 点击下方表格的“垃圾桶图标”，将所有条目删除
    \item 在浏览器中打开\uhref{https://slzfw.sdsmu.edu.cn:8800/home}{https://slzfw.sdsmu.edu.cn:8800/home$^㊕$}，点击右上角的“在线信息”→“一键下线”按钮
    ...
    \item 设置：
          \begin{enumerate}
              \item 常规设置：
                    \begin{enumerate}
                        ...
                        \item 将邮箱账户关联至“山东第二医科大学”app（\textbf{必须使用电脑端浏览器进行}）
                              \begin{enumerate}
                                  \item 打开\uhref{https://cas.sdsmu.edu.cn:4102}{安全中心}→点击上方菜单“账户安全”→“安全设置”→
              ...
              \item 安全设置：
                        ...
                        \item 配置第三方邮件客户端（以Thunderbird为例\footnotemark）
                              ...
                              \begin{enumerate}
                                  ...
                                  \item 配置如下：\\
                                        ...
                                        专用密码：aaaa aaaa aaaa aaaa（空格为美观需要，实际输入请删除）\\
                    ...
                    \end{enumerate}
          \end{enumerate}
    \item 其他：每60天需要更改一次密码（密码建议14～16位，含大小写、数字、特殊字符如下划线“\_”等）
\end{enumerate}
...
\label{dry_machine}
\begin{enumerate}
    \item 注册等步骤详见\uref{washing_machine_f}
...
\begin{enumerate}
    ...
    \item 点击“申请宿舍入住”，按提示操作即可\footnotemark
          \footnotetext{注意，调宿前请先与自己的导员沟通并取得要调换的宿舍的成员的同意。}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item 其他要求同上文所述，略
    \item \textbf{注意}：所有申请表及相关文件\uul{有效期均不超过一年且不可跨学年}
\end{enumerate}

\subsection[空闲教室查询]{空闲教室查询}
\label{spare_classroom}
\begin{enumerate}
    \item 根据下文\uref{cas_system}的CAS认证系统登录教程，登录山二医app（下载链接见\uref{sdsmu_app}）
...
\begin{enumerate}
    ...
    \item 教室维修（浮烟山校区）：
          ...
          \end{enumerate}
    \item 宿舍维修（虞河校区）：在宿管处填写报修单据并送往物业维修中心（位置见地图\uref{common_locations_yuhe}）
...
\begin{enumerate}
    ...
    \item 填写教务系统（详细步骤如下）
          \begin{enumerate}
              \item 进入教务系统（仅校园网，详情参见\uref{academic_affairs_system}）
...
\begin{enumerate}
    ...
    \item 前往教室E区2层（见\uref{map_fuyanshan_teach_building}）靠近A区处的“教室管理中心”签字盖章
...
\begin{enumerate}
    ...
    \item 字体下载：\uhref{http://dwbgs.sdsmu.edu.cn/_upload/article/files/d9/bb/f9e99e374220b0e81c81a5c47065/2ac83882-cd93-4d4d-bfa1-33023db6dc50.rar}{字体（方正小标宋简体、仿宋GB2312、楷体GB2312）.rar}
    \item 具体要求：
          \begin{enumerate}
              \item 纸张大小：A4纸
...
\begin{enumerate}
    \item 用途：学工系统主要用于晚点名、返校信息填报等日常工作\footnotemark
          \footnotetext{原“请假审批”、“外出审批”工作已基本转移至“钉钉”，教程参见\uref{leave_dingtalk}。}
    \item 使用方式：
          \begin{enumerate}
...
\subsection[教务系统]{\textbf{\uul{教务系统}}}
...
\begin{enumerate}
    ...
    \item \textbf{注意：}仅限校内访问，如需在外使用教务系统，参见\uref{cas_system}条目
\end{enumerate}

\subsection[CAS资源访问控制系统（校内VPN）]{\textbf{\uul{CAS资源访问控制系统（校内VPN）}}\footnotemark}
...
\begin{enumerate}
    ...
    \item 说明：（也称CAS认证系统，智慧校园系统）本系统主要用于\textbf{在校外访问校内网络信息资源}，如：教务系统（查成绩、课表）、知网、临床医学虚拟仿真实验中心\footnotemark 等
          \footnotetext{查阅文献推荐使用CARSI系统，速度快效果好，教程详见\uref{carsi_system}。}
...
\subsection[CARSI系统]{\textbf{\uul{CARSI系统}}}
\label{carsi_system}
\begin{enumerate}
...
\subsection[人脸识别录入]{人脸识别录入}
\label{face_system}
\begin{enumerate}
    \item 用途：宿舍、图书馆门禁等
    \item 录入/更新步骤：
          \begin{enumerate}
              \item 网页：登录\uhref{https://portal.sdsmu.edu.cn}{https://portal.sdsmu.edu.cn$^㊕$}→点击“人脸采集”→按提示操作上传照片
...
\begin{enumerate}
    ...
    \item 线下打印
          \begin{enumerate}
              \item 携带身份证、手机前往D区的打印机，详情位置见\uref{common_locations_fuyanshan}
              \item 按照机器的说明填写相关信息并打印（无纸时请联系工作人员）
          \end{enumerate}
    \item 线上下载
          \begin{enumerate}
              \item 学籍证明：请按校务行教程\uref{cert_prover}申请下载至邮箱中
              \item 个人档案：按照校务行教程\uref{cert_prover}申请并下载，自行前往打印店打印即可（请注意及时删除打印完毕的文件以免泄漏隐私）
          \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item 按照\uref{cas_system}的说明登录“山东第二医科大学App”
...
\begin{enumerate}
    ...
    \item 在预定时间段\footnotemark 前往场馆，向工作人员出示二维码即可
          \footnotetext{场馆开放时间见\uref{sports_center_operating_hours}。}
    \item 费用（校内学生）：
          \begin{enumerate}
              \item 健体中心：3元/2小时
...
%尽量手动控制此节的排版……
\subsection[公交信息与乘车指南]{公交信息与乘车指南}
\label{free_bus}
运营时间通常为06:30～19:00
...
在学校提交相关信息后，可通过\textbf{与提交信息相符}的手机号注册认证“\textbf{潍坊公交大学生乘车码}”微信小程序免费乘车（仅限政策试行期间注册的用户）。如需更改绑定的手机号，需向学校提出申请。

如需单独的公交指南信息，敬请移步以下链接
\begin{enumerate}
    \item 在线预览：\uhref{https://docs.qq.com/s/ETcQ-ZFSrSsh6MK9bm773q}{腾讯文档（在“指南分片”文件夹中）}
    \item 下载：\uhref{https://gitee.com/LinkChou/SDSMU-Companion-Part/releases}{Gitee}或\uhref{https://github.com/Mikachu2333/SDSMU-Companion-Part/releases}{GitHub}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item 详细要求：参见《山东第二医科大学——PPT制作指南》，因内容过多此处不做详述
          \begin{enumerate}
              \item 在线预览：\uhref{https://docs.qq.com/s/ETcQ-ZFSrSsh6MK9bm773q}{腾讯文档（在“指南分片”文件夹中）}
              \item Gitee下载：\uhref{https://gitee.com/LinkChou/SDSMU-Companion-Part/releases}{发行版}
              \item GitHub下载：\uhref{https://github.com/Mikachu2333/SDSMU-Companion-Part/releases}{Releases}
          \end{enumerate}
    \item 另注：“潍医红”色值：CYMK(10,100,100,40) 或 RGB(154,0,4)
\end{enumerate}

\subsection[家长、校友进校参观指南]{家长、校友进校参观指南\footnotemark}
\footnotetext{注：本节仅适用浮烟山校区；虞河校区为开放校区，除宿舍、教学楼外，均可随时参观。}
\begin{enumerate}
...
\subsection[正版系统与正版软件激活、系统重装教程]{正版系统与正版软件激活、系统重装教程}
\begin{enumerate}
    \item 官网网址：\uhref{https://ms.sdsmu.edu.cn}{https://ms.sdsmu.edu.cn$^㊕$}
    \item 学校购买的常用正版系统及软件含
          \begin{enumerate}
              \item Windows 8、Windows 8.1、Windows 10、Windows 11
              \item Office 2021、Office 2019、Office 2016、Office 2013
              \item Office 2021 for Mac、Office 2019 for Mac
              \item WPS 365、WPS 365 for Mac
          \end{enumerate}
    \item 学校推荐的其他免费软件或镜像含：
          \begin{enumerate}
              \item Ventoy（重装工具类，\textbf{推荐}）\\
                    官网：\uhref{https://www.ventoy.net/cn}{https://www.ventoy.net/cn}\\
                    下载（NJU镜像源）：\\
                    \uhref{https://mirrors.nju.edu.cn/github-release/ventoy/Ventoy/LatestRelease}{https://mirrors.nju.edu.cn/github-release/ventoy/Ventoy/LatestRelease}
              \item Rufus（重装工具类）\\
                    官网：\uhref{https://rufus.ie/zh}{https://rufus.ie/zh}\\
                    下载（FossHub源）：\uhref{https://www.fosshub.com/Rufus.html}{https://www.fosshub.com/Rufus.html}
              \item 微PE工具箱（镜像类）\\
                    官网：\uhref{https://www.wepe.com.cn}{https://www.wepe.com.cn}\\
                    下载（SDU源）：\uhref{https://mirrors.sdu.edu.cn/wepe}{https://mirrors.sdu.edu.cn/wepe}
          \end{enumerate}
    \item 激活：
          \begin{enumerate}
              \item 激活软件下载链接见\uhref{https://ms.sdsmu.edu.cn/soft/detail/12}{https://ms.sdsmu.edu.cn/soft/detail/12$^㊕$}
              \item 激活教程请参考官网\uhref{https://ms.sdsmu.edu.cn/help/detail/2}{GP激活客户端使用说明$^㊕$}
          \end{enumerate}
    \item 重装系统教程参考\\
          \textbf{警告：无经验者不推荐，\uul{重装前务必备份资料}，文章仅供参考，请以实际情况为准}\\
          \uhref{https://www.ventoy.net/cn/doc_start.html}{https://www.ventoy.net/cn/doc\_start.html}\\
          \uhref{https://firpe.cn/page-335}{https://firpe.cn/page-335}等
    \item 本人推荐的其他相关软件及超链接：\\
          \textbf{警告：盲目使用\uul{必定}导致蓝屏及系统损毁、资料损失，无相关经验者切勿试险！}
          \begin{enumerate}
              \item Dism++\\
                    相关链接：\uhref{https://github.com/Chuyu-Team/Dism-Multi-language}{https://github.com/Chuyu-Team/Dism-Multi-language}
              \item TexLive\\
                    相关链接：\uhref{https://mirrors.ustc.edu.cn/CTAN/systems/texlive/Images}{https://mirrors.ustc.edu.cn/CTAN/systems/texlive/Images}
              \item FirPE\\
                    相关链接：\uhref{https://box.nju.edu.cn/d/63ebb2f83dd54e65acac}{https://box.nju.edu.cn/d/63ebb2f83dd54e65acac}
              \item EdgelessPE\\
                    相关链接：\uhref{https://zfile.edgeless.top}{https://zfile.edgeless.top}
          \end{enumerate}
    \item \textbf{注意：}上述操作必须在校园网环境下进行！\textbf{\uul{租借账号、售卖、为他人获取相关校内资源等属于违法行为！}}
\end{enumerate}

\subsection[身份证补办、临时身份证明、居住证申请、无犯罪记录证明]{身份证补办、临时身份证明、居住证申请、无犯罪记录证明}
\label{lost_identity}
\begin{enumerate}
    \item 方式：线下、微信公众号“潍坊微警务”
    \item 用途：申请无犯罪记录、居住证、补办身份证、申请临时身份证等
    ...
    \item 相关申请的步骤：
          \begin{enumerate}
              ...
              \item →选择相关需要办理的事项并按要求填写后提交（临时身份证明可直接下载到）
          \end{enumerate}
\end{enumerate}